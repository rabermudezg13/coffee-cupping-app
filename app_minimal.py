...
def show_coffee_bags(auth_manager):
    ...
    if user_bags:
        st.markdown(f"#### Your Coffee Bags ({len(user_bags)})")
        for bag in user_bags[:3]:  # Show last 3 bags
            with st.expander(f"📦 {bag.get('coffeeName', 'Unknown Coffee')} - {bag.get('rating', 0)}⭐"):
                ...
                with col2:
                    st.write(f"**Grind Type:** {bag.get('grindType', 'N/A')}")
                    st.write(f"**Preparation:** {bag.get('preparationMethod', 'N/A')}")
                    st.write(f"**Cost:** ${bag.get('cost', 0):.2f}")
                    st.write(f"**Roast Date:** {bag.get('roastDate', 'N/A')}")
                
                col3, col4 = st.columns(2)
                with col3:
                    recommend = "✅ Yes" if bag.get('wouldRecommend') else "❌ No"
                    st.write(f"**Would Recommend:** {recommend}")
                with col4:
                    buy_again = "✅ Yes" if bag.get('wouldBuyAgain') else "❌ No"
                    st.write(f"**Would Buy Again:** {buy_again}")
    ...
    with st.form("coffee_bag_form"):
        ...
        col3, col4 = st.columns(2)
        with col3:
            rating = st.slider("Rating", 1, 5, 4, help="Rate this coffee (1-5 stars)")
        ...
        uploaded_photo = st.file_uploader(
            "Upload a photo (optional)",
            type=['png', 'jpg', 'jpeg'],
            help="Upload a photo of the coffee bag or brewing"
        )
        
        # Privacy Settings
        col5, col6 = st.columns(2)
        with col5:
            is_public = st.checkbox("Make this public", value=True)
        with col6:
            is_anonymous = st.checkbox("Post as Anonymous", value=False)
            ...
            if coffee_name and origin and roast_level and grind_type and preparation_method:
                ...
                with st.spinner("Saving coffee bag..."):
                    ...
                    if bag_id:
                        ...
                        st.balloons()
                        st.info("Your coffee bag has been added to your collection!")
                    else:
...
def show_collaborative_cupping(auth_manager):
    ...
    # Simple tabs for minimal version
                ...
                if user_response and user_response.get('response') == 'accept':
                    with st.form(f"eval_{invitation['invitationId']}"):
                        ...
                        if st.form_submit_button("Submit Evaluation"):
                            evaluation_data = {'overall_score': overall_score, 'flavor_notes': flavor_notes}
